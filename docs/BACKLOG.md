...
| 64 | **Cola de aprobación asíncrona para acciones hacia fuera** (`git push`, Commit + PR, Auto-PR, merge del worktree al PASS): el agente sigue trabajando y las acciones externas se encolan para aprobarlas en bloque cuando al usuario le convenga. Hoy solo hay dos posturas y ninguna intermedia: el modo `ask` bloquea el run en un `.sheet` modal por cada tool (`ChatView:1856`, `ChatViewModel:1132`), o se auto-aprueba (`acceptEdits` por defecto en `AppSettings:84`, `/auto` → `bypassPermissions` en `ChatView:2018`, loops fijos a `acceptEdits` en `LoopRunner:409`). **Requisito de diseño: encolar una instantánea inmutable, no un comando.** Al encolar hay que fijar un SHA (commit o `stash create` del árbol en ese momento) y que la aprobación publique *ese* SHA. Hoy ninguna de las dos acciones lo hace: `commitAndPR` commitea el árbol de trabajo tal como esté al invocarse (`CodeModeView:1174-1176`) y `CodeGit.push` resuelve `HEAD` en el momento de la llamada (`CodeGit:507-509`), así que si el agente sigue trabajando mientras la acción espera aprobación, se publicaría un diff que el humano nunca vio — que es justo lo que haría este mecanismo *peor* que la puerta síncrona: aparenta revisión sin darla. El worktree ya da esa instantánea de forma natural (el tip de su rama es el SHA revisado). **Límite explícito: solo acciones hacia fuera.** No aplicar a `Edit`/`Write`/`Bash` — ahí los pasos siguientes releen lo escrito, así que diferir la acción real obligaría a devolver al agente un resultado ficticio y el resto del run se construiría sobre una vista falsa del repo. Para esa mitad ya tenemos la respuesta correcta: aislamiento en worktree (resultado real, merge diferido). | UX/Servicios | L | Inspirado en los Gatekeepers de Cloudflare OS (ver #63). Su diagnóstico —"el human-in-the-loop síncrono acaba empujando a todo el mundo a auto-approve"— describe literalmente el modo `ask` actual. Numerado 64 por ser posterior; la prioridad es la de su sección (P2). |
...
| 63 | **"Browse / Research" nativo** (idea, 2026-08-07): un modo de lectura/verificación de páginas propio de Coral, no atado a la búsqueda de cada CLI. Backend enchufable: Playwright local (coherente con local-first) y/o **Cloudflare browser-run "Kitesurf"** (CDP compatible Puppeteer/Playwright/MCP; barato, agente-first; beta gratis) como opción cloud. OJO: Kitesurf es *fetch/render*, NO buscador → necesita un buscador real (Brave/Bing API) delante que dé las URLs. Encaja como MCP server opcional; el cloud choca con "sin cuenta", así que ofrecerlo junto al local, nunca como vía única. Inspiración adicional de **Cloudflare OS** (Gatekeepers/permibilidad por capacidad, "gadgets", BYO-model con gasto por app/rol) — competidor enterprise, solo conceptos. Cloudflare OS se liberó como open source el 2026-08-05; la pieza de Gatekeepers que sí nos aplica se sacó a su propio ítem, **#64**. | Producto/Servicios | L |
