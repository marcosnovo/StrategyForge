...
- **Visor de diff SOTA en Código (nuevo, ago-2026):** paridad de UX de revisión con Cursor/Zed/GitHub — resaltado de sintaxis + **word-level intraline** en cada línea, **vista unificada ↔ side-by-side** conmutable, **navegación por hunks** (▲/▼ + contador n/total, salta solo bloques con cambio), **"Visto" por archivo** con barra de progreso de revisión, y los hallazgos del **verificador independiente ahora fijados al diff** (marca en el gutter por severidad + clic→scroll a la línea). Modelo de hunk estructurado y probado (base para stage/revert por hunk, pendiente). *(Cierra el hueco de "el visor de diff se sentía plano" frente a Devin Review/Amp Oracle: ahora la revisión cross-familia se lee dentro del código, no en una lista aparte.)*
