...
extension L10n {
    ...
    static let loopStrings: [String: (en: String, es: String)] = [
        ...
        "loop.editor.verifyCommand.label": ("verify command (mechanical gate)", "comando de verificación (gate mecánico)"),
        "loop.editor.verifyCommand.placeholder": ("e.g. swift build && swift test — exit 0 = pass",
                                                  "p. ej. swift build && swift test — exit 0 = pasa"),
        "loop.editor.verifyCommand.why": ("Runs first each turn; a non-zero exit is an instant FAIL, no LLM opinion — the strongest gate against a loop calling itself done too early.",
                                          "Se ejecuta primero cada turno; un exit ≠0 es FAIL inmediato, sin opinión del LLM — el gate más fuerte contra que el loop se declare hecho antes de tiempo."),
        "loop.editor.mustHold.label": ("must still hold", "debe seguir siendo cierto"),
        "loop.editor.mustHold.placeholder": ("One counter-condition per line, e.g. no test deleted or skipped",
                                             "Una contra-condición por línea, p. ej. ningún test borrado ni saltado"),
        "loop.editor.mustHold.why": ("A guardrail against the loop gaming its goal: a PASS needs the goal met AND these still true.",
                                     "Un guardarraíl contra que el loop se engañe: un PASS necesita el objetivo cumplido Y que esto siga siendo cierto."),
