...
enum DesignSystem: String, CaseIterable, Identifiable, Sendable {
    ...
    var palette: Palette {
        ...
        case .classic:
            ...
            return Palette(
                // Near-WHITE warm paper ramp (R>=G>=B — no green/slate cast). appBg is almost
                // white so the app reads clean; columns/rail sit a whisper below and wells a
                // touch deeper, with #FFF cards still reading as lifted sheets. The flat tokens
                // ARE the surface now (glass materials dropped on light), so this is what shows.
                // (Design pass 4, 2026-08-05 — the "más blanco / más limpio" fix.)
                appBg: hex("FCFCFB", "0C1315"), columnBg: hex("F7F6F4", "121E21"),
                cardBg: hex("FFFFFF", "16262A"), insetBg: hex("EFEEEA", "1B2E33"),
                railBg: hex("F2F1EE", "081014"),
