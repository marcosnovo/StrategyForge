...
enum L10n {
    ...
    static let strings: [String: (en: String, es: String)] = [
        ...
        "chat.continueRun": ("Continue", "Continuar"),
        "chat.interrupted": ("The run stopped before finishing (it may have been interrupted). Continue to pick up where it left off.",
                             "La ejecución se detuvo antes de terminar (puede haberse interrumpido). Continúa para seguir donde lo dejó."),
        ...
        "chat.blocked": ("Connect %@ to launch — it's not set up yet, so this chat can't run on it.",
                         "Conecta %@ para lanzar — aún no está configurado, así que este chat no puede ejecutarse con él."),
        ...
        // ⌘K command palette
        "palette.placeholder": ("Search chats and actions…", "Buscar chats y acciones…"),
        "palette.section.actions": ("Actions", "Acciones"),
        "palette.section.chats": ("Chats", "Chats"),
        "palette.go": ("Go to %@", "Ir a %@"),
        ...
        "appearance.auto": ("Auto (system)", "Auto (sistema)"),
        "appearance.light": ("Light", "Claro"),
        "appearance.dark": ("Dark", "Oscuro"),
        ...
        "empty.greeting": ("Ready when you are", "Listo cuando quieras"),
        ...
        "rail.map": ("Code Map", "Mapa del código"),
        "rail.code.help": ("Solo coding on one repo", "Programar en solitario sobre un repo"),
        "rail.map.help": ("Turn a repo into a knowledge graph", "Convierte un repo en un grafo de conocimiento"),
        ...
        "splash.models": ("Updating model catalog…", "Actualizando catálogo de modelos…"),
        ...
        "map.group.kind": ("Type", "Tipo"),
        "map.group.local": ("Local", "Local"),
        ...
        "rail.images": ("Images", "Imágenes"),
        "rail.images.help": ("Generate images from a prompt", "Genera imágenes desde un texto"),
        "images.title": ("Image Studio", "Estudio de imágenes"),
        "images.prompt": ("Prompt", "Descripción"),
        "images.prompt.placeholder": ("Describe the image you want…", "Describe la imagen que quieres…"),
        "images.provider": ("Provider", "Proveedor"),
        "images.aspect": ("Aspect", "Formato"),
        "images.aspect.square": ("Square", "Cuadrado"),
        ...
        "images.regenerate": ("Regenerate", "Regenerar"),
        "images.empty": ("Your generated image will appear here.", "Tu imagen generada aparecerá aquí."),
        "images.error.decode": ("The service returned data that isn't a valid image.", "El servicio devolvió datos que no son una imagen válida."),
        "images.needKey.title": ("Add an image-provider API key", "Añade una API key de un proveedor de imágenes"),
        "images.needKey.detail": ("Image generation uses OpenAI (gpt-image-1) or Google (Imagen) via an API key. Claude has no image generation. Add a key in Services.",
                                  "La generación de imágenes usa OpenAI (gpt-image-1) o Google (Imagen) con una API key. Claude no genera imágenes. Añade una clave en Servicios."),
        "map.title": ("Code Map", "Mapa del código"),
        "map.open.html": ("Interactive", "Interactivo"),
        "map.chat": ("Open in chat", "Abrir en chat"),
        ...
        "map.watch.help": ("Rebuild the map automatically as the code changes (graphify watch).",
                           "Reconstruye el mapa automáticamente cuando cambia el código (graphify watch)."),
        "map.explain": ("Explanation", "Explicación"),
        "map.explain.intro": ("“%@” is a %@ at %@, in the “%@” family, with %d connections.",
                              "«%@» es un %@ en %@, de la familia «%@», con %d conexiones."),
        "map.explain.hub": ("It's a well-connected hub of the codebase.", "Es un hub muy conectado del código."),
        "map.explain.connects": ("It's mainly connected to: %@.", "Se conecta sobre todo con: %@."),
        "map.explain.details": ("— Technical detail (graphify) —", "— Detalle técnico (graphify) —"),
        "map.node.title": ("Node", "Nodo"),
        "map.node.connections": ("%d connections", "%d conexiones"),
        "map.node.neighbors": ("Connected to (%d)", "Conectado con (%d)"),
        "map.communities": ("Communities", "Comunidades"),
        "map.explaining": ("Explaining…", "Explicando…"),
        "approaches.label": ("Approaches", "Enfoques"),
        "approaches.on": ("Approaches on — I'll lay out a few options first, then you pick.",
                          "Enfoques activado — te propondré varias opciones antes de implementar."),
        "approaches.off": ("Approaches off.", "Enfoques desactivado."),
        "grill.on": ("Grill me on — I'll ask clarifying questions before implementing.",
                     "Pregúntame activado — te haré preguntas antes de implementar."),
        "grill.off": ("Grill me off.", "Pregúntame desactivado."),
        "images.needKeyInline": ("To generate images, add an OpenAI or Google API key in Services (Claude has no image generation). Then try again right here.",
                                 "Para generar imágenes, añade una API key de OpenAI o Google en Servicios (Claude no genera imágenes). Luego inténtalo de nuevo aquí mismo."),
        "approaches.help": ("Propose approaches first: before implementing, the agent lays out a few distinct approaches with tradeoffs so you pick one.",
                            "Propón enfoques primero: antes de implementar, el agente plantea varios enfoques distintos con sus ventajas para que elijas uno."),
        "arena.recommend": ("Recommended:", "Recomendado:"),
        "arena.recommend.quality": ("best quality (judge)", "mejor calidad (juez)"),
        "arena.recommend.cheapest": ("cheapest", "más barato"),
        "arena.recommend.cheaper": ("%@ is cheaper (~%@)", "%@ es más barato (~%@)"),
        "isolate.label": ("Isolate", "Aislar"),
        "isolate.help": ("Run in an isolated git worktree — the agent's edits don't touch your working tree until you review and merge. Recommended with the most permissive mode.",
                         "Ejecuta en un worktree git aislado — las ediciones del agente no tocan tu árbol de trabajo hasta que revises y fusiones. Recomendado con el modo más permisivo."),
        ...
        "isolate.verified.panel": ("Verified by %d judges", "Verificado por %d jueces"),
        "blast.contained": ("Easy to undo", "Fácil de deshacer"),
        "blast.wide": ("Affects a lot", "Afecta a mucho"),
        "blast.irreversible": ("Hard to undo", "Difícil de deshacer"),
        "blast.contained.help": ("Reversible and isolated — a bad merge is a quick revert.",
                                 "Reversible y aislado — un merge malo se revierte rápido."),
        "blast.wide.help": ("Reversible but touches a shared/wide surface — review the checks and the trajectory before merging.",
                            "Reversible pero toca una superficie amplia/compartida — revisa los checks y la trayectoria antes de fusionar."),
        "blast.irreversible.help": ("Hard to undo (migrations, deletions, prod, money) — a human must review before merge.",
                                    "Difícil de deshacer (migraciones, borrados, prod, dinero) — un humano debe revisar antes de fusionar."),
        "blast.why": ("How risky to undo — %@", "Riesgo al deshacer — %@"),
        "blast.merge.confirm.title": ("Merge an irreversible change?", "¿Fusionar un cambio irreversible?"),
        "blast.merge.confirm.body": ("This change is hard to undo (%@). Make sure you've reviewed the diff — this lane never merges automatically.",
                                     "Este cambio es difícil de deshacer (%@). Asegúrate de haber revisado el diff — este carril nunca se fusiona automáticamente."),
        "blast.merge.confirm.go": ("Merge anyway", "Fusionar igualmente"),
        "isolate.issues": ("%d issues", "%d problemas"),
        "isolate.verify.fail": ("Verify failed", "Falló la verificación"),
        "isolate.merge": ("Merge", "Fusionar"),
        "isolate.discard": ("Discard", "Descartar"),
        "ship.via": ("Deploy to %@", "Desplegar a %@"),
        "ship.help": ("Publish this repo with a deploy CLI you already have (Vercel · Netlify · Cloudflare). Coral runs it in the repo and pulls the live URL.",
                      "Publica este repo con una CLI de despliegue que ya tienes (Vercel · Netlify · Cloudflare). Coral la ejecuta en el repo y saca la URL en vivo."),
        "ship.confirm.title": ("Deploy publicly via %@?", "¿Desplegar públicamente con %@?"),
        "ship.confirm.body": ("This publishes the repo live using the CLI's own login. It's outward-facing — make sure this is the deploy you want.",
                              "Esto publica el repo en vivo usando el login de la propia CLI. Es una acción externa — asegúrate de que es el despliegue que quieres."),
        "ship.confirm.deploy": ("Deploy", "Desplegar"),
        ...
        "grill.label": ("Grill me", "Pregúntame"),
        "grill.help": ("Grill me first: before doing any work, the agent asks the sharp edge-case questions this task needs — and waits for your answers.",
                       "Pregúntame primero: antes de trabajar, el agente hace las preguntas clave de casos límite que necesita esta tarea — y espera tus respuestas."),
        "map.node.open": ("Open file", "Abrir archivo"),
        "map.node.explain": ("Explain", "Explicar"),
        "map.node.ask": ("Ask", "Preguntar"),
        "map.node.path": ("Path…", "Ruta…"),
        "map.ask.draft": ("Explain `%@`%@ — what it does, what calls it, and its role in the codebase.",
                          "Explícame `%@`%@ — qué hace, qué lo llama y su papel en el código."),
        "map.path.title": ("Shortest path between two symbols", "Camino más corto entre dos símbolos"),
        "map.path.from": ("From", "Desde"),
        "map.path.to": ("To", "Hasta"),
        "map.path.find": ("Find path", "Buscar ruta"),
        "map.injected": ("Fed to agents", "Se inyecta a los agentes"),
        "map.injected.help": ("On the first turn of a chat/Code session on this repo, Coral injects a compact version of this map so the agent starts with structure instead of re-reading files. Estimated savings appear in Usage.",
                              "En el primer turno de una sesión de chat/Code sobre este repo, Coral inyecta una versión compacta de este mapa para que el agente empiece con la estructura en vez de releer archivos. El ahorro estimado aparece en Uso."),
        "usage.map.title": ("Code map savings", "Ahorro del mapa de código"),
        "usage.map.saved": ("~%@ tokens saved this session", "~%@ tokens ahorrados esta sesión"),
        "usage.map.detail": ("Injected into %d session(s) · ~%@ · estimated, not billed", "Inyectado en %d sesión(es) · ~%@ · estimado, no facturado"),
        "map.run": ("Build map", "Construir mapa"),
        "map.rerun": ("Rebuild", "Reconstruir"),
        "map.pick.repo": ("Choose repo…", "Elegir repo…"),
        "map.github": ("From GitHub", "Desde GitHub"),
        "map.github.title": ("Map a repo from GitHub", "Mapear un repo de GitHub"),
        "map.github.placeholder": ("github.com/owner/repo  or  owner/repo", "github.com/owner/repo  o  owner/repo"),
        "map.github.map": ("Clone & map", "Clonar y mapear"),
        "map.github.mine": ("Your repositories", "Tus repositorios"),
        "map.github.loading": ("Loading your repos…", "Cargando tus repos…"),
        "map.github.none": ("No repositories found.", "No se encontraron repositorios."),
        ...
        "map.ready.title": ("Ready to build", "Listo para construir"),
        "map.ready.desc": ("Build a knowledge graph of the selected repo — deterministic and token-cheap in light mode. Coral prepares its engine on the first build.",
                           "Construye un grafo de conocimiento del repo seleccionado — determinista y barato en tokens en modo ligero. Coral prepara su motor en la primera construcción."),
        "map.running": ("Building the map…", "Construyendo el mapa…"),
        "map.preparing": ("Preparing the map engine… (one-time)", "Preparando el motor de mapas… (una vez)"),
        "map.needsPython.title": ("Command Line Tools needed", "Faltan las Command Line Tools"),
        "map.needsPython.desc": ("Coral installs the graphify engine into its own isolated Python environment — it just needs Apple's Command Line Tools (python3), once.",
                                 "Coral instala el motor graphify en su propio entorno Python aislado — solo necesita una vez las Command Line Tools de Apple (python3)."),
        "map.installCLT": ("Install Command Line Tools", "Instalar Command Line Tools"),
        "map.failed": ("Couldn't build the map", "No se pudo construir el mapa"),
        "map.retry": ("Try again", "Reintentar"),
        "map.stat.clusters": ("%d clusters", "%d clústeres"),
        "map.stat.nodes": ("%d nodes", "%d nodos"),
        "map.stat.edges": ("%d edges", "%d conexiones"),
        "map.showing": ("Showing top %d of %d", "Mostrando %d de %d"),
        "map.html.title": ("Interactive graph", "Grafo interactivo"),
        "rail.loops": ("Loops", "Bucles"),
        "sidebar.newShort": ("New", "Nuevo"),
        "rail.more": ("More", "Más"),
        "rail.section.work": ("Work", "Trabajar"),
        "rail.section.compare": ("Compare", "Comparar"),
        "code.launcher.title": ("Code", "Código"),
        "code.list.title": ("Code", "Código"),
        "code.list.new": ("New code session", "Nueva sesión de código"),
        "code.list.search": ("Search code sessions", "Buscar sesiones de código"),
        "code.list.empty": ("No code sessions yet — connect a repo to start.", "Aún no hay sesiones de código — conecta un repo para empezar."),
        "code.list.repo": ("Repository", "Repositorio"),
        "code.list.repo.all": ("All repositories", "Todos los repos"),
        "code.list.group.repo": ("Repository", "Repositorio"),
        ...
        "provider.issue.migrateAntigravity.title": ("Gemini's free CLI was retired", "El CLI gratuito de Gemini fue retirado"),
        "provider.issue.migrateAntigravity.detail": ("Google no longer supports the free Gemini CLI, and Antigravity is a desktop app Coral can't drive in the background. The one path that works is a Gemini API key — paste it in the card above.",
                                                     "Google ya no admite el CLI gratuito de Gemini, y Antigravity es una app de escritorio que Coral no puede ejecutar en segundo plano. Lo único que funciona es una API key de Gemini — pégala en la tarjeta de arriba."),
        "provider.fix.antigravity": ("Get Antigravity", "Instalar Antigravity"),
        "provider.fix.geminiKey": ("Get a Gemini API key", "Obtener API key de Gemini"),
        "provider.issue.invalidKey.title": ("API key not accepted", "API key no aceptada"),
        "provider.issue.invalidKey.detail": ("Google rejected this Gemini API key. Check you pasted it whole and that it's enabled in Google AI Studio, then paste it again above.",
                                             "Google rechazó esta API key de Gemini. Comprueba que la pegaste completa y que está habilitada en Google AI Studio, y vuelve a pegarla arriba."),
        "provider.gemini.key.title": ("GEMINI API KEY", "API KEY DE GEMINI"),
        "provider.gemini.key.note": ("Google retired the free Gemini CLI login, so an API key from Google AI Studio is the only way Coral can run Gemini. It's used automatically for every Gemini chat and team role.",
                                     "Google retiró el inicio de sesión del CLI gratuito de Gemini, así que una API key de Google AI Studio es la única forma de que Coral ejecute Gemini. Se usa automáticamente en cada chat y rol de equipo con Gemini."),
        "provider.gemini.key.saved": ("Gemini API key saved.", "API key de Gemini guardada."),
        "provider.gemini.key.present": ("A key is saved in your Keychain. Gemini is ready to use.",
                                        "Hay una key guardada en tu Llavero. Gemini está listo para usar."),
        "provider.gemini.key.hint": ("Stored in your Keychain, never on disk or synced.",
                                     "Se guarda en tu Llavero, nunca en disco ni sincronizado."),
        ...
        "usage.week.resetsIn": ("Weekly window resets in %@", "La ventana semanal se reinicia en %@"),
        "activity.usage.resetsIn": ("· resets in %@", "· reinicio en %@"),
        ...
        "usage.gemini.tokens": ("TOKENS USED", "TOKENS USADOS"),
        "usage.gemini.note": ("Google doesn't expose a usage limit for Gemini API keys — only the tokens you've spent are shown here.",
                              "Google no expone un límite de uso para las API keys de Gemini — aquí solo se muestran los tokens que has gastado."),
        "usage.favorite.on": ("Pinned to the sidebar — tap to unpin", "Fijado en la barra lateral — toca para quitar"),
        "usage.favorite.off": ("Pin to the sidebar usage panel", "Fijar en el panel de uso de la barra lateral"),
        "usage.favorite.full": ("Up to 3 pinned — unpin one first", "Máximo 3 fijados — quita uno primero"),
        ...
        // Chat list: filter / group / sort
        "chat.filter.help": ("Filter, group and sort the chat list", "Filtra, agrupa y ordena la lista de chats"),
        "chat.filter.status": ("Status", "Estado"),
        "chat.filter.status.all": ("All", "Todos"),
        "chat.filter.status.active": ("Active", "Activos"),
        "chat.filter.provider": ("Provider", "Proveedor"),
        "chat.filter.provider.all": ("All providers", "Todos los proveedores"),
        "chat.groupBy": ("Group by", "Agrupar por"),
        "chat.sortBy": ("Sort by", "Ordenar por"),
        "chat.group.date": ("Date", "Fecha"),
        "chat.group.folder": ("Folder", "Carpeta"),
        "chat.group.state": ("State", "Estado"),
        "chat.group.none": ("None", "Ninguno"),
        "chat.sort.recency": ("Recency", "Recientes"),
        "chat.sort.name": ("Name", "Nombre"),
        "chat.sort.oldest": ("Oldest first", "Más antiguos primero"),
        "chat.group.today": ("Today", "Hoy"),
        "chat.group.yesterday": ("Yesterday", "Ayer"),
        "chat.group.thisWeek": ("This week", "Esta semana"),
        "chat.group.thisMonth": ("This month", "Este mes"),
        "chat.group.older": ("Older", "Más antiguo"),
        "chat.group.noRepo": ("No repo", "Sin repo"),
        "chat.group.running": ("Running", "En marcha"),
        "chat.group.active": ("Active", "Con actividad"),
        "chat.group.new": ("New", "Nuevos"),
        ...
        "onboard.intro": (
            "Just ask. Coral assembles the right team of agents for your task automatically — you don't set anything up first. Redesign the team whenever you want.",
            "Solo pregunta. Coral monta el equipo de agentes adecuado para tu tarea automáticamente — no configuras nada antes. Rediseña el equipo cuando quieras."),
        "onboard.step1.title": ("1. Ask anything", "1. Pregunta lo que sea"),
        "onboard.step1.desc": ("Type your question or task — like any chat. Optionally point it at a folder of code, documents or notes.",
                               "Escribe tu pregunta o tarea — como cualquier chat. Opcionalmente apúntalo a una carpeta de código, documentos o notas."),
        "onboard.step2.title": ("2. Coral picks the agents", "2. Coral elige los agentes"),
        "onboard.step2.desc": ("Coral chooses the right team for the job — one agent for a simple question, a specialized team for a bigger one. This is what a single assistant can't do.",
                               "Coral elige el equipo adecuado — un agente para una pregunta simple, un equipo especializado para algo mayor. Esto es lo que un solo asistente no puede hacer."),
        "onboard.step3.title": ("3. Redesign anytime", "3. Rediseña cuando quieras"),
        "onboard.step3.desc": ("Happy? Just keep chatting. Want control? Open the team to change roles, models, providers, or turn it into an autonomous loop — all one tap away.",
                               "¿Contento? Sigue chateando. ¿Quieres control? Abre el equipo para cambiar roles, modelos, proveedores, o convertirlo en un loop autónomo — todo a un toque."),
        "onboard.note": (
        ...
        // Paged onboarding (Antigravity-style: Welcome → Appearance → Connect).
        "onboard.next": ("Next", "Siguiente"),
        "onboard.previous": ("Previous", "Anterior"),
        "onboard.finish": ("Finish", "Terminar"),
        "onboard.step.appearance.title": ("Choose your appearance", "Elige la apariencia"),
        "onboard.step.appearance.desc": ("Coral follows your system by default — you can change it anytime in Settings.",
                                         "Coral sigue tu sistema por defecto — puedes cambiarlo cuando quieras en Ajustes."),
        "onboard.step.connect.title": ("Connect your tools", "Conecta tus herramientas"),
        "onboard.step.connect.desc": ("Sign in with your own plans — no API keys, no metering. Claude is the default engine; add Codex or Gemini for cross-provider teams.",
                                      "Inicia sesión con tus planes — sin claves API ni medición. Claude es el motor por defecto; añade Codex o Gemini para equipos multi-proveedor."),
        "onboard.connect.action": ("Connect", "Conectar"),
        "onboard.connected": ("Connected", "Conectado"),
        "onboard.github.title": ("GitHub", "GitHub"),
        "onboard.github.desc": ("Clone and open your repositories from inside Coral.",
                                "Clona y abre tus repositorios desde dentro de Coral."),
        // The wedge — what makes Coral different from the metered, closed, cloud tools.
        "onboard.wedge.title": ("Yours, not rented", "Tuyo, no alquilado"),
        "onboard.wedge.byo.title": ("No metering", "Sin medición"),
        "onboard.wedge.byo.desc": ("Runs on the AI plans you already pay for — no per-token bill on top, no markup.",
                                   "Corre sobre los planes de IA que ya pagas — sin factura por tokens encima, sin margen."),
        "onboard.wedge.private.title": ("Your keys, your Mac", "Tus llaves, tu Mac"),
        "onboard.wedge.private.desc": ("No account, no server. Your code and logins never leave this machine.",
                                       "Sin cuenta, sin servidor. Tu código y tus accesos no salen de esta máquina."),
        "onboard.wedge.mix.title": ("Mix providers", "Mezcla proveedores"),
        "onboard.wedge.mix.desc": ("Claude, Codex and Gemini on one team — plan with one, build with another, review with a third.",
                                   "Claude, Codex y Gemini en un equipo — planea con uno, construye con otro, revisa con un tercero."),
        "onboard.wedge.open": ("Open source (MIT) — audit exactly what it does with your credentials.",
                               "Código abierto (MIT) — audita exactamente qué hace con tus credenciales."),

        // MARK: Empty states
        "empty.editor.title": ("Start a chat with your own agent team", "Empieza un chat con tu equipo de agentes"),
        "empty.editor.desc": (
            "Just ask. Coral picks the right agents for your task — one for a quick question, a team for a bigger one. Point it at a folder anytime; redesign the team whenever you want.",
        ...
        "context.panel.title": ("Context", "Contexto"),
        "context.results": ("RESULTS", "RESULTADOS"),
        "context.sources": ("SOURCES", "FUENTES"),
        "context.empty": ("Sources you attach and files the team produces will appear here.",
                          "Aquí aparecerán las fuentes que adjuntes y los archivos que produzca el equipo."),
        ...
        "chat.answeredByTeam.help": ("A team of agents produced this — tap to see who did what.",
                                     "Un equipo de agentes produjo esto — toca para ver quién hizo qué."),
        "chat.loopSuggestion": ("This looks repeatable — run it on a loop that verifies itself?",
                                "Esto parece repetible — ¿ejecutarlo en un loop que se verifica solo?"),
        "chat.loopSuggestion.cta": ("Make a loop", "Crear loop"),
        "chat.repoSuggestion": ("Working on code? Attach a folder so the agents can read and edit it.",
                                "¿Trabajando en código? Adjunta una carpeta para que los agentes puedan leerlo y editarlo."),
        "chat.repoSuggestion.cta": ("Attach a folder", "Adjuntar carpeta"),
        "chat.secondOpinion": ("2nd opinion · %@", "2ª opinión · %@"),
        "chat.secondOpinion.help": ("Ask %@ the same question and compare the two answers side by side.",
                                    "Pregunta lo mismo a %@ y compara las dos respuestas una al lado de la otra."),
        "chat.secondOpinion.header": ("**Second opinion — %@**", "**Segunda opinión — %@**"),
        ...
        "banner.memorySaveFailed": ("Couldn’t save memory: %@", "No se pudo guardar la memoria: %@"),
        ...
        // Hygiene: forgetting, contradictions, audit
        "memory.attn.pending": ("to review", "por revisar"),
        "memory.attn.stale": ("stale", "caducos"),
        "memory.attn.conflicts": ("conflicts", "conflictos"),
        "memory.staleness.fresh": ("Fresh", "Vigente"),
        "memory.staleness.aging": ("Aging", "Envejeciendo"),
        "memory.staleness.stale": ("Stale — old and unused", "Caduco — viejo y sin uso"),
        "memory.stale.notice": ("This learning is old and hasn't been used in a while. Keep it if it's still true, or forget it so it stops taking up context.",
                                "Este aprendizaje es viejo y hace tiempo que no se usa. Consérvalo si sigue siendo cierto, u olvídalo para que deje de ocupar contexto."),
        "memory.stale.keep": ("Keep", "Conservar"),
        "memory.stale.forget": ("Forget", "Olvidar"),
        "memory.conflict.notice": ("This disagrees with another learning about the same subject: “%@”. Both may have been right in different contexts — you decide which to keep.",
                                   "Esto contradice a otro aprendizaje sobre el mismo tema: «%@». Ambos pudieron ser ciertos en contextos distintos — decides tú cuál conservar."),
        "memory.conflict.open": ("Open the other", "Abrir el otro"),
        "memory.audit.title": ("History", "Historial"),
        "memory.audit.empty": ("No changes recorded yet.", "Aún no hay cambios registrados."),
        "memory.audit.added": ("Added", "Añadido"),
        "memory.audit.approved": ("Approved", "Aprobado"),
        "memory.audit.edited": ("Edited", "Editado"),
        "memory.audit.pinned": ("Pinned", "Fijado"),
        "memory.audit.unpinned": ("Unpinned", "Desfijado"),
        "memory.audit.deleted": ("Deleted", "Borrado"),
        "memory.audit.forgotten": ("Forgotten", "Olvidado"),
        "memory.approve": ("Review", "Revisar"),
        "memory.pending.help": ("Auto-harvested — pending your review. It is NOT injected into agents until you approve it (so a wrong lesson can't compound across runs).",
                                "Auto-cosechado — pendiente de tu revisión. NO se inyecta a los agentes hasta que lo apruebes (así una lección errónea no se propaga entre ejecuciones)."),
        "memory.kind.pattern": ("Pattern", "Patrón"),
        "memory.kind.decision": ("Decision", "Decisión"),
        "memory.kind.mistake": ("Pitfall", "Error a evitar"),
        "memory.filter.all": ("All", "Todos"),
        "memory.empty.title": ("Knowledge that carries over", "Conocimiento que perdura"),
        "memory.empty.desc": ("Patterns, decisions and pitfalls learned from past runs — injected into future teams' CLAUDE.md so they improve over time. Add one, or import a repo's STATE.md.",
                              "Patrones, decisiones y errores aprendidos en runs pasados — se inyectan en el CLAUDE.md de futuros equipos para que mejoren con el tiempo. Añade uno o importa el STATE.md de un repo."),
        "memory.pin": ("Pin", "Fijar"),
        "memory.unpin": ("Unpin", "Desfijar"),
        "memory.field.title": ("Title", "Título"),
        "memory.field.body": ("Detail", "Detalle"),
        "memory.field.tags": ("Tags (comma-separated)", "Etiquetas (separadas por comas)"),
        "memory.source.manual": ("Added manually", "Añadido manualmente"),
        "memory.source.review": ("From a code review", "De una revisión de código"),
        "memory.source.mission": ("From a mission report", "De un informe de misión"),
        "memory.source.state": ("Imported from STATE.md", "Importado de STATE.md"),
        "memory.import.message": ("Choose a repo's STATE.md to import its learnings", "Elige el STATE.md de un repo para importar sus aprendizajes"),
        "memory.import.empty": ("No learnings found in that file.", "No se encontraron aprendizajes en ese archivo."),
        "memory.import.done": ("Imported %d learnings.", "Importados %d aprendizajes."),
        "review.saveMemory": ("Save to memory", "Guardar en memoria"),
        "review.saveMemoryHint": ("Distil these findings into the knowledge base, so future teams avoid the same pitfalls.",
        ...
        "arena.task.placeholder": ("Describe the task to send to every provider…", "Describe la tarea que enviar a cada proveedor…"),
        "arena.entrants": ("Competitors", "Competidores"),
        "arena.noProviders": ("Connect at least two AI providers to run an arena.", "Conecta al menos dos proveedores de IA para correr una arena."),
        "arena.run": ("Run arena", "Correr arena"),
        "arena.cancel": ("Cancel", "Cancelar"),
        "arena.running": ("Running the task on every competitor…", "Ejecutando la tarea en cada competidor…"),
        "arena.results": ("Results", "Resultados"),
        "arena.winner": ("Winner", "Ganador"),
        "arena.pickWinner": ("Pick as winner", "Elegir como ganador"),
        "arena.copy": ("Copy", "Copiar"),
        "arena.copied": ("Answer copied.", "Respuesta copiada."),
        "arena.sendToChat": ("Continue in a chat", "Seguir en un chat"),
        "arena.error": ("This competitor failed.", "Este competidor falló."),
        // Arena — Teams mode (whole strategies compete)
        "arena.mode.models": ("Models", "Modelos"),
        "arena.mode.teams": ("Teams", "Equipos"),
        // Two explicit axes (fixes the models-vs-teams confusion).
        "arena.axis.who": ("Competitors", "Compiten"),
        "arena.axis.compare": ("Compare on", "Comparan"),
        "arena.who.models": ("Models", "Modelos"),
        "arena.who.teams": ("Teams", "Equipos"),
        "arena.compare.answers": ("A task (answers)", "Una tarea (respuestas)"),
        "arena.compare.code": ("Your code (edits)", "Tu código (ediciones)"),
        "arena.combo.models.answers": ("Several models answer the same task — keep the best.",
                                       "Varios modelos responden la misma tarea — quédate con la mejor."),
        "arena.combo.models.code": ("Several models edit your repo (each in an isolated worktree) — compare the diffs.",
                                    "Varios modelos editan tu repo (cada uno en un worktree aislado) — compara los diffs."),
        "arena.combo.teams.answers": ("Whole team strategies answer the same task, live — keep the best.",
                                      "Estrategias de equipo enteras responden la misma tarea, en vivo — quédate con la mejor."),
        "arena.combo.teams.code": ("Whole teams edit your repo (each in an isolated worktree, native) — compare the diffs.",
                                   "Equipos enteros editan tu repo (cada uno en un worktree aislado, nativo) — compara los diffs."),
        "arena.teams.subtitle": ("Race whole team strategies (multi-provider) on one task — the agentic arena only Coral can run.",
                                 "Compite estrategias de equipo enteras (multi-proveedor) en una tarea — la arena agéntica que solo Coral puede correr."),
        "arena.teams.pick": ("Competing teams", "Equipos que compiten"),
        "arena.teams.hint": ("Pick 2+ teams. Each runs the full task read-only (no repo edits) so they can't interfere with each other.",
                             "Elige 2+ equipos. Cada uno corre la tarea completa en solo-lectura (sin editar el repo) para que no se interfieran."),
        "arena.teams.makeup": ("%d agents", "%d agentes"),
        "arena.teams.cost.title": ("Run the team arena?", "¿Correr la arena de equipos?"),
        "arena.teams.cost.body": ("This runs %d full multi-agent executions on this task. It spends real tokens on every provider involved and can take a few minutes — you can watch each team's progress live and cancel anytime.",
                                  "Esto ejecuta %d equipos multi-agente completos en esta tarea. Gasta tokens reales en cada proveedor implicado y puede tardar unos minutos — verás el progreso de cada equipo en vivo y puedes cancelar cuando quieras."),
        "arena.teams.cost.run": ("Run it", "Correr"),
        "arena.state.queued": ("Queued", "En cola"),
        "arena.state.running": ("Working", "Trabajando"),
        "arena.state.done": ("Done", "Listo"),
        ...
        // Arena — Code mode (providers edit in isolated worktrees; compare diffs)
        "arena.mode.code": ("Code", "Código"),
        "arena.code.subtitle": ("Each provider does the coding task in its own isolated git worktree — compare the diffs and apply the best.",
                               "Cada proveedor hace la tarea de código en su propio worktree git aislado — compara los diffs y aplica el mejor."),
        "arena.code.noRepo": ("No repo selected", "Sin repo seleccionado"),
        "arena.code.chooseRepo": ("Choose repo", "Elegir repo"),
        "arena.code.recent": ("Your repos", "Tus repos"),
        "arena.code.fromGitHub": ("Get a GitHub repo…", "Traer un repo de GitHub…"),
        "arena.code.chooseFolder": ("Choose a folder…", "Elegir una carpeta…"),
        "arena.code.safety": ("Each provider edits in an isolated worktree on its own branch — your working tree isn't touched until you apply a winner.",
                              "Cada proveedor edita en un worktree aislado en su propia rama — tu árbol de trabajo no se toca hasta que apliques un ganador."),
        "arena.code.cost.title": ("Run the code arena?", "¿Correr la arena de código?"),
        "arena.code.cost.body": ("This lets %d providers EDIT files (each in an isolated worktree — your tree is safe) and spends real tokens. It can take a while; you can cancel anytime.",
                                 "Esto deja que %d proveedores EDITEN ficheros (cada uno en un worktree aislado — tu árbol está a salvo) y gasta tokens reales. Puede tardar; puedes cancelar cuando quieras."),
        "arena.code.cost.run": ("Run it", "Correr"),
        "arena.code.editing": ("Editing in an isolated worktree…", "Editando en un worktree aislado…"),
        "arena.code.changed": ("%d files changed", "%d ficheros cambiados"),
        "arena.code.noChanges": ("No changes produced.", "No produjo cambios."),
        "arena.code.apply": ("Apply to repo", "Aplicar al repo"),
        "arena.code.applied": ("Merged the winner into your repo.", "Fusionado el ganador en tu repo."),
        "arena.code.applyFailed": ("Couldn't merge: %@", "No se pudo fusionar: %@"),
        "arena.code.discard": ("Discard all", "Descartar todo"),
        ...
        "services.firstrun": ("Connect at least one AI to run your teams.",
                              "Conecta al menos una IA para ejecutar tus equipos."),
        "provider.usedBy": ("Used by", "Usado por"),
        "provider.usedBy.empty": ("No chats use %@ yet.", "Ningún chat usa %@ todavía."),
        "provider.usedBy.start": ("Start a chat on %@", "Empezar un chat en %@"),
        "provider.usedBy.chats": ("%d chats", "%d chats"),
        "provider.usedBy.teams": ("%d teams", "%d equipos"),
        "provider.advanced": ("Advanced · CLI & model", "Avanzado · CLI y modelo"),
        // Future-provider roadmap voting (Connections → "Coming soon").
        "future.vote": ("Vote", "Votar"),
        "future.voted": ("Voted", "Votado"),
        "future.footer": ("Not live yet. We add providers by votes — saved on this Mac, shared anonymously only if usage sharing is on.",
                          "Aún no disponibles. Añadimos proveedores por votos — se guardan en este Mac y se comparten de forma anónima solo si tienes activado compartir uso."),
        "future.voted.count": ("You've voted for %d.", "Has votado por %d."),
        "future.request": ("Request another…", "Pedir otro…"),
        "future.request.title": ("Request a provider", "Pedir un proveedor"),
        "future.request.hint": ("Tell us what to build next — this goes straight to the team's list.",
                                "Dinos qué construir a continuación — esto va directo a la lista del equipo."),
        "future.request.placeholder": ("Provider or model name", "Nombre del proveedor o modelo"),
        "future.request.send": ("Send request", "Enviar"),
        "future.request.done": ("Thanks — request noted.", "Gracias — petición registrada."),
        // Future-provider taglines (the one-line "why").
        "future.kimi.tagline": ("Cheap frontier coding · ~1M context + vision", "Frontier barato · ~1M de contexto + visión"),
        "future.glm.tagline": ("Cheap coding plan · a Claude Code drop-in", "Plan de código barato · compatible con Claude Code"),
        "future.qwen.tagline": ("Open-weights · strong coder, easy to add", "Pesos abiertos · buen coder, fácil de añadir"),
        "future.deepseek.tagline": ("Rock-bottom price · open, great value", "Precio mínimo · abierto, gran relación calidad-precio"),
        "future.grok.tagline": ("Fast, coding-tuned · from xAI", "Rápido, afinado para código · de xAI"),
        "future.mistral.tagline": ("EU-hosted · privacy-friendly coding", "Alojado en la UE · código respetuoso con la privacidad"),
        "future.local.tagline": ("Run offline on your Mac · fully private", "Ejecútalo sin conexión en tu Mac · totalmente privado"),
        ...
        "provider.node.title": ("Node.js isn't installed", "Node.js no está instalado"),
        "provider.node.desc": ("Coral installs provider CLIs with npm, which needs Node.js. Install it once and Coral takes it from there.",
                               "Coral instala las CLIs de los proveedores con npm, que necesita Node.js. Instálalo una vez y Coral sigue solo."),
        "provider.node.brew": ("Install Node with Homebrew", "Instalar Node con Homebrew"),
        "provider.node.installing": ("Installing Node.js with Homebrew… this can take a couple of minutes.",
                                     "Instalando Node.js con Homebrew… puede tardar un par de minutos."),
        "provider.node.download": ("Download Node.js", "Descargar Node.js"),
        "provider.node.manual": ("No Homebrew found. Download the Node.js installer, run it, then press Connect again.",
                                 "No se encontró Homebrew. Descarga el instalador de Node.js, ejecútalo y pulsa Conectar de nuevo."),
        "provider.signin.browserHint": ("We opened your browser — sign in there. This window finishes automatically.",
                                        "Hemos abierto tu navegador — inicia sesión ahí. Esta ventana termina sola."),
        ...
        "slash.message": ("Message another chat", "Enviar a otro chat"),
        ...
        "mode.plan": ("Plan", "Plan"),
        "mode.auto": ("Full autonomy", "Autonomía total"),
        ...
        "chat.empty": ("Ask a question, review a document, or start a task. Your team works right here.",
                       "Haz una pregunta, revisa un documento o empieza una tarea. Tu equipo trabaja aquí mismo."),
        ...
        "chat.team.count": ("A lead agent + %lld teammates", "Un agente líder + %lld compañeros"),
        "chat.team.solo": ("A single agent handles everything", "Un solo agente lo hace todo"),
        "chat.delegating": ("Handing off to %@", "Pasando a %@"),
        ...
        "chat.allowAlways.confirmMsg": ("From now on this chat can edit files and run commands without asking — until you switch it off in the header.",
                                        "A partir de ahora este chat puede editar archivos y ejecutar comandos sin preguntar — hasta que lo desactives en la cabecera."),
        ...
        "chat.mixedProviders": ("A few of these agents will run on Claude for now — the others aren't connected yet.",
                                "Algunos de estos agentes correrán en Claude por ahora — los demás aún no están conectados."),
        ...
        "code.reviewed": ("Reviewed", "Revisado"),
        ...
        "code.checkpoint": ("Checkpoint", "Punto"),
        "code.checkpoint.help": ("Snapshot the working tree, or rewind to a past snapshot",
                                 "Captura el árbol de trabajo, o vuelve a una captura anterior"),
        "code.largeDiff": ("Large diff — %lld lines. Rendering is paused to keep things smooth.",
                           "Diff grande — %lld líneas. El render está en pausa para no ralentizar."),
        "code.largeDiff.show": ("Show diff anyway", "Mostrar el diff igualmente"),
        "code.checkpoint.capture": ("Capture checkpoint now", "Capturar punto ahora"),
        "code.checkpoint.rewindTo": ("Rewind to…", "Volver a…"),
        "code.checkpoint.rewind": ("Rewind", "Volver"),
        "code.checkpoint.label": ("Checkpoint %lld", "Punto %lld"),
        "code.checkpoint.saved": ("Checkpoint captured", "Punto capturado"),
        "code.checkpoint.restored": ("Rewound to checkpoint", "Vuelto al punto"),
        "code.checkpoint.empty": ("Nothing to snapshot — the working tree is clean.",
                                  "Nada que capturar — el árbol de trabajo está limpio."),
        ...
        "checklist.help": ("Launch checklist — empty repo to shipped", "Checklist de lanzamiento — de repo vacío a publicado"),
        "checklist.delete": ("Delete this checklist", "Borrar este checklist"),
        "checklist.empty.title": ("Launch checklist", "Checklist de lanzamiento"),
        "checklist.empty.desc": ("Pick a template to track every step from empty repo to shipped — the boring parts are where launches go wrong.",
                                 "Elige una plantilla para seguir cada paso de repo vacío a publicado — las partes aburridas son donde fallan los lanzamientos."),
        ...
        "trajectory.title": ("Trajectory — who did what", "Trayectoria — quién hizo qué"),
        "trajectory.redone": ("%lld steps were paid for twice — a teammate redid work another already did.",
                              "%lld pasos se pagaron dos veces — un compañero rehizo trabajo que otro ya hizo."),
        "trajectory.clean": ("No duplicated work — the agents didn't re-walk each other's steps.",
                             "Sin trabajo duplicado — los agentes no repitieron pasos de otros."),
        "trajectory.empty": ("No tool steps yet.", "Aún no hay pasos de herramienta."),
        "trajectory.redone.tag": ("REDONE", "REPETIDO"),
        "activity.redone.title": ("Redone work", "Trabajo repetido"),
        "activity.redone.help": ("The same tool + target run more than once — often a second agent redoing what the first already did, because agents can't share a window. Invisible in the answer, only visible here.",
                                 "La misma herramienta + objetivo ejecutada más de una vez — a menudo un segundo agente rehaciendo lo que el primero ya hizo, porque los agentes no comparten ventana. Invisible en la respuesta, solo visible aquí."),
        ...
        // MARK: Cross-chat messaging (PeerMessage)
        "peer.send.open": ("Message another chat…", "Enviar a otro chat…"),
        "peer.send.title": ("Message another chat", "Enviar a otro chat"),
        "peer.send.caption": ("Send a short note to one of your other chats. Text only — it never carries this chat's transcript, files, or context. The other chat reads it as a message from this one, not as an instruction from you.",
                              "Envía una nota corta a otro de tus chats. Solo texto — nunca lleva la transcripción, los archivos ni el contexto de este chat. El otro chat lo lee como un mensaje de este, no como una instrucción tuya."),
        "peer.send.to": ("To", "Para"),
        "peer.send.placeholder": ("What should the other chat know?", "¿Qué debería saber el otro chat?"),
        "peer.send.action": ("Send", "Enviar"),
        "peer.send.noTargets": ("You don't have another chat to message yet.",
                                "Todavía no tienes otro chat al que escribir."),
        "peer.sent": ("Message sent to %@.", "Mensaje enviado a %@."),
        "peer.held": ("Message waiting for your approval in %@.",
                      "Mensaje esperando tu aprobación en %@."),
        "peer.from": ("Message from %@", "Mensaje de %@"),
        "peer.reply": ("Reply", "Responder"),
        "peer.inbox.title": ("Messages from other chats", "Mensajes de otros chats"),
        "peer.inbox.approve": ("Deliver", "Entregar"),
        "peer.inbox.deny": ("Discard", "Descartar"),
        "peer.inbox.empty": ("Nothing waiting.", "Nada en espera."),

        "peer.inbound.accept": ("Deliver", "Entregar"),
        "peer.inbound.hold": ("Ask me first", "Preguntarme antes"),
        "peer.inbound.refuse": ("Ignore", "Ignorar"),
        "peer.inbound.accept.blurb": ("A message from another chat starts a turn right away.",
                                      "Un mensaje de otro chat inicia un turno de inmediato."),
        "peer.inbound.hold.blurb": ("Messages wait until you approve them.",
                                    "Los mensajes esperan a que los apruebes."),
        "peer.inbound.refuse.blurb": ("Messages from other chats are dropped.",
                                      "Los mensajes de otros chats se descartan."),
        "settings.peerInbound": ("Messages from your other chats", "Mensajes de tus otros chats"),
        "settings.peerInbound.caption": ("A message from another chat starts a real turn: it spends tokens and can touch the repo. “Ask me first” is the safe default; an unattended loop needs “Deliver” to react at all.",
                                         "Un mensaje de otro chat inicia un turno real: gasta tokens y puede tocar el repositorio. «Preguntarme antes» es el valor seguro; un loop desatendido necesita «Entregar» para poder reaccionar."),

        "peer.refusal.empty": ("There was nothing to send.", "No había nada que enviar."),
        "peer.refusal.selfAddressed": ("A chat can't message itself.", "Un chat no puede escribirse a sí mismo."),
        "peer.refusal.unknownPeer": ("That chat no longer exists.", "Ese chat ya no existe."),
        "peer.refusal.inboundRefuse": ("That chat isn't accepting messages.", "Ese chat no acepta mensajes."),
        "peer.refusal.duplicate": ("You just sent that same message.", "Acabas de enviar ese mismo mensaje."),
        "peer.refusal.rateLimited": ("Too many messages to that chat just now — try again shortly.",
                                     "Demasiados mensajes a ese chat ahora mismo — inténtalo en un momento."),
        "peer.refusal.queueFull": ("That chat has too many unread messages.",
                                   "Ese chat tiene demasiados mensajes sin leer."),
        ...
        "eval.subtitle": ("Test scenarios your team is scored against — measure it before you trust it.",
                          "Escenarios de prueba con los que se puntúa tu equipo — mídelo antes de confiar en él."),
        // Tool unit tests (article #5)
        "toolcheck.title": ("Tool checks", "Pruebas de tools"),
        "toolcheck.subtitle": ("Deterministic checks that run a command and assert on the output — no model, so they're fast and repeatable. Most agent bugs are tool bugs.",
                               "Pruebas deterministas que ejecutan un comando y comprueban su salida — sin modelo, rápidas y repetibles. La mayoría de bugs de agentes son bugs de tools."),
        "toolcheck.empty": ("Add a check to verify a tool works before the team relies on it — e.g. a curl to an MCP endpoint that must return a field, or a skill script that must exit 0.",
                            "Añade una prueba para verificar que una tool funciona antes de que el equipo dependa de ella — p.ej. un curl a un endpoint MCP que debe devolver un campo, o un script de skill que debe salir con 0."),
        "toolcheck.count": ("%lld checks", "%lld pruebas"),
        "toolcheck.allPass": ("All pass", "Todas pasan"),
        "toolcheck.someFail": ("Some fail", "Algunas fallan"),
        "toolcheck.name": ("What it checks", "Qué comprueba"),
        "toolcheck.command": ("Command to run", "Comando a ejecutar"),
        "toolcheck.expectContains": ("Output must contain…", "La salida debe contener…"),
        "toolcheck.exitZero": ("Exit 0", "Sale 0"),
        "toolcheck.add": ("Add check", "Añadir prueba"),
        "toolcheck.run": ("Run checks", "Ejecutar pruebas"),
        "toolcheck.doneBanner": ("%lld of %lld checks passed", "%lld de %lld pruebas pasaron"),
        "eval.empty": ("No scenarios yet. Generate a suite from the team's purpose, then run it.",
                       "Aún no hay escenarios. Genera un suite a partir del propósito del equipo y ejecútalo."),
        "eval.count": ("%lld scenarios", "%lld escenarios"),
        "eval.generate": ("Generate scenarios", "Generar escenarios"),
        ...
        // Node/edge cost lens
        "lens.title": ("Cost lens", "Lente de coste"),
        "lens.subtitle": ("Spend that buys thinking (nodes) vs routing/fan-out (edges you can cut)",
                          "Gasto que compra pensamiento (nodos) vs enrutado/fan-out (aristas que puedes recortar)"),
        "lens.nodes": ("%lld model calls / turn", "%lld llamadas al modelo / turno"),
        "lens.saving": ("save %@ / turn", "ahorras %@ / turno"),
        "lens.clean": ("Lean — spend goes to thinking, not overhead.", "Ajustado — el gasto va a pensar, no a overhead."),
        "lens.finding.singleWorker": ("One worker behind an orchestrator: plan + synthesize are 2 routing calls a Solo/Executor would skip (%@).",
                                      "Un solo worker tras un orquestador: plan + síntesis son 2 llamadas de enrutado que un Solo/Ejecutor evitaría (%@)."),
        "lens.finding.fanout": ("%1$@ ×%2$lld on a frontier model — parallel drones rarely need the top tier; a cheaper one saves %3$@.",
                                "%1$@ ×%2$lld en un modelo frontera — los drones en paralelo rara vez necesitan el tier top; uno más barato ahorra %3$@."),
        "lens.finding.excess": ("%1$@ ×%2$lld — likely more parallelism than the task needs.",
                                "%1$@ ×%2$lld — probablemente más paralelismo del que la tarea necesita."),
        ...
        "eval.progress": ("Scoring %lld of %lld…", "Puntuando %lld de %lld…"),
        // Auto-improve (RAI)
        "eval.improve": ("Auto-improve", "Auto-mejorar"),
        ...
        "eval.improve.failed": ("Auto-improve couldn't run — check the team's provider is connected.",
                                "No se pudo auto-mejorar — comprueba que el proveedor del equipo está conectado."),
        "eval.improve.title": ("Proposed improvements", "Mejoras propuestas"),
        "eval.improve.editsCount": ("%lld lever change(s)", "%lld cambio(s) de palanca"),
        "eval.improve.noEdits": ("No changes were needed — the team already meets its probes, or no safe edit was found.",
                                 "No hicieron falta cambios — el equipo ya cumple sus probes, o no se encontró una edición segura."),
        "eval.improve.apply": ("Apply to team", "Aplicar al equipo"),
        "eval.improve.discard": ("Discard", "Descartar"),
        "eval.improve.applied": ("Applied %lld change(s) to the team.", "Aplicados %lld cambio(s) al equipo."),
        "eval.threshold": ("Ready when ≥ %lld%% pass.", "Listo cuando pasa ≥ %lld%%."),
        "eval.ready": ("Ready", "Listo"),
        "eval.notReady": ("Below threshold", "Por debajo del umbral"),
        "eval.pass": ("passed", "superado"),
        "eval.fail": ("failed", "fallado"),
        "eval.category.answersCorrectly": ("Answers correctly", "Responde bien"),
        ...
        // Evals: rubric, trajectory, regression, calibration
        "eval.dim.correctness": ("correctness", "corrección"),
        "eval.dim.grounding": ("grounding", "fundamentación"),
        "eval.dim.safety": ("safety", "seguridad"),
        "eval.path.ok": ("Path ✓", "Camino ✓"),
        "eval.path.bad": ("Path ✗", "Camino ✗"),
        "eval.path.help": ("Whether the team took the expected path (subagents/tools), not just the right answer.",
                           "Si el equipo tomó el camino esperado (subagentes/tools), no solo la respuesta correcta."),
        "eval.regressed": ("%lld regressed", "%lld en regresión"),
        "eval.fixed": ("%lld fixed", "%lld corregidos"),
        "eval.regression.since": ("vs last run", "vs la última vez"),
        "eval.noRegression": ("No changes vs last run", "Sin cambios vs la última vez"),
        "eval.calibrate.agree": ("Agree with the judge", "De acuerdo con el juez"),
        "eval.calibrate.disagree": ("Disagree — override the judge", "En desacuerdo — anular al juez"),
        "eval.overridden": ("overridden", "anulado"),
