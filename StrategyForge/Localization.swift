...
enum L10n {
    ...
    static let strings: [String: (en: String, es: String)] = [
        ...
        "settings.connected.caption": ("Run your chats on different AIs using your own subscription logins — no API keys. Install each provider's CLI and sign in there; the app detects it automatically.",
                                       "Ejecuta tus chats en distintas IAs usando tus propios inicios de sesión — sin claves API. Instala el CLI de cada proveedor e inicia sesión ahí; la app lo detecta automáticamente."),
        ...
        "provider.openai.connect": ("Install the Codex CLI (`npm i -g @openai/codex`) and run `codex` to sign in with your ChatGPT account. Running chats on it arrives in the next phase.",
                                    "Instala el CLI de Codex (`npm i -g @openai/codex`) y ejecuta `codex` para iniciar sesión con tu cuenta de ChatGPT. Ejecutar chats con él llega en la próxima fase."),
        "provider.gemini.connect": ("Install the Gemini CLI (`npm i -g @google/gemini-cli`) and run `gemini` to sign in with your Google account. Running chats on it arrives in the next phase.",
                                    "Instala el CLI de Gemini (`npm i -g @google/gemini-cli`) y ejecuta `gemini` para iniciar sesión con tu cuenta de Google. Ejecutar chats con él llega en la próxima fase."),
